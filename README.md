...
- `demo-member`: 회원 도메인, 패스워드 해싱, 활성화 코드 발급/검증, JPA 어댑터.
- `demo-auth`: 로그인/토큰 발급·재발급, 리프레시 저장소(JPA·테스트용 InMemory), HMAC 서명 토큰 프로바이더.
- `demo-api`: 실행 모듈. REST API, 시큐리티 설정(401 엔트리포인트/토큰 필터), 예외 처리, H2 데모 설정.
...
2) 활성화 `POST /api/members/activate` -> 코드/만료 검증 -> 활성화 상태 전환  
3) 로그인 `POST /api/auth/login` -> 패스워드 검증 -> 액세스/리프레시 토큰 발급(HMAC)  
4) 토큰 재발급 `POST /api/auth/refresh` -> 리프레시 검증/저장소 확인 -> 새 토큰 교체  
5) 보호된 API 예시 `GET /api/members/{id}` -> `Authorization: Bearer <access>` 필요  
...
- 토큰 저장소: 기본 JPA 저장소(`JpaRefreshTokenStore`), 테스트 프로파일에서는 인메모리(`InMemoryRefreshTokenStore`).
- 추가 문서: `docs/auth-flow.md`에서 가입/활성화/로그인/인가 시퀀스를 Mermaid로 시각화.
